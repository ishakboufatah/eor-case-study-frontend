          ...
          document.querySelector('#'+this.eortype.replaceAll(' ', '_')),
     {
      ...
      axios({
        ...
        response.data.forEach((item) => {
          if ((Number.isFinite(item.salinity_of_formation_water_ppm) == true)&& (Number.isFinite(item.incremental_eor_recovery_factor_fraction) == true)) { if ((item.eor_type !== null)){
            rowDataa.push({ field: item.field + " " + item.pool_name, Recovery_factor: item.incremental_eor_recovery_factor_fraction, Sal: item.salinity_of_formation_water_ppm, eor_type: item.eor_type })
