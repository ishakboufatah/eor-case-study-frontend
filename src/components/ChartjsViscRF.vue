      ...
      axios({
        ...
        response.data.forEach((item) => {
          if ((Number.isFinite(item.oil_viscosity_15c_cp) == true)&& (Number.isFinite(item.incremental_eor_recovery_factor_fraction) == true)) { if ((item.eor_type !== null)){
            rowDataa.push({ field: item.field + " " + item.pool_name, Recovery_factor: item.incremental_eor_recovery_factor_fraction, Visc: item.oil_viscosity_15c_cp, eor_type: item.eor_type })
